...
def filter_out_non_before_after_images(clusters):
    ...
    for cluster, filenames in clusters.items():
        ...
        for filename in filenames:
            timestamp = get_image_timestamp(os.path.join("UPLOAD_FOLDER", filename))
            if timestamp:
...
def load_images(image_directory):   
    ...
    for filename in os.listdir(image_directory):
        if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".JPEG"): # add any other file types if necessary
...
def cluster_images(image_directory):
    ...
    # Assuming 'features' is a list of feature vectors and 'labels' are their cluster labels
    ...
    # For a more complex but potentially more informative view, you can use t-SNE instead
    # tsne = TSNE(n_components=2, perplexity=30, n_iter=300)
    # reduced_features = tsne.fit_transform(features)

    # Plotting
    plt.figure(figsize=(10, 10))
    scatter = plt.scatter(reduced_features[:, 0], reduced_features[:, 1], c=labels, cmap='rainbow')
    ...
    for single_cluster_key, single_cluster_features in single_feature_clusters.items():
        ...
        if matches_within_5_percent:
            ...
            if distances[1][0] / closest_distance > 1.08:
    ...
    clusters = filter_out_non_before_after_images(clusters)
